...
def import_dataset_kaggle():
    
    # the dataset parameters ( https://www.kaggle.com/datasets/AnalyzeBoston/crimes-in-boston )
    crime_dataset_param = 'AnalyzeBoston/crimes-in-boston'

    # the download directory
    download_path_crime = os.path.join(current_directory, 'data')

    # download function
    kaggle.api.dataset_download_files(crime_dataset_param, path=download_path_crime, unzip=True)


def load_data():

    file_path_main = os.path.join(current_directory, 'data/crime.csv')
    file_path_codes = os.path.join(current_directory, 'data/offense_codes.csv')

    crime_data=pd.read_csv(
        file_path_main, encoding='windows-1252')
    
    offense_data=pd.read_csv(
        file_path_codes ,encoding='windows-1252')
